...
├── cockpit (main card)
│   ├── cockpit-header   (brand + contextual label + strategy-tabs → Institutional Credit route link)
│   └── cockpit-body
│       ├── cockpit-chart-pane
...
- The cockpit header keeps the interactive strategy `tablist` in the right-side cluster and appends an [`Institutional Credit`](TransactionCockpit.tsx) link to `?view=advisor` immediately after the WETH/wstETH tabs. The `0x` product mark and variant-specific contextual label remain separate header elements; the mobile header wraps the whole cluster without separating the route link from its tabs.
